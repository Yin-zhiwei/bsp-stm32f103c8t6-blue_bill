...
if PLATFORM == 'gcc':
    ...
    LFLAGS = DEVICE + ' -Wl,--gc-sections,-Map=out/rtthread.map,-cref,-u,Reset_Handler -T board/linker_scripts/link.lds'
    ...
    if USE_DEFAULT_TOOLCHAIN:
        ...
        bin_path = os.path.join('out', 'rtthread.bin').replace('/', '\\')
        axf_path = os.path.join('out', 'rtthread.axf').replace('/', '\\')
        elf_path = os.path.join('out', 'rtthread.elf').replace('/', '\\')

        # Define POST_ACTION with PowerShell's Copy-Item cmdlet
        copy_command = f'powershell -Command "Copy-Item \\"{axf_path}\\" \\"{elf_path}\\" -Force"'

        POST_ACTION = OBJCPY + f' -O binary "$TARGET" "{bin_path}"\n'
        POST_ACTION += SIZE + f' "$TARGET"\n'

        # Add the Copy-Item command to POST_ACTION
        POST_ACTION += copy_command + '\n'

        # Debugging information
        print("Executing command:", copy_command)
